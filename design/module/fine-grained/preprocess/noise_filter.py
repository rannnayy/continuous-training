...
def read_file(input_file):
    ...
    df.columns = ["ts_record","size","queue_len","prev_queue_len_1","prev_queue_len_2","prev_queue_len_3","prev_latency_1","prev_latency_2","prev_latency_3","prev_throughput_1","prev_throughput_2","prev_throughput_3","latency","reject"]
...
def start_processing(input_path, output_dir): 
...
# 8. Write data as labeled dataset
    ...
    important_columns = ["ts_record","size","queue_len","prev_queue_len_1","prev_queue_len_2","prev_queue_len_3","prev_latency_1","prev_latency_2","prev_latency_3","prev_throughput_1","prev_throughput_2","prev_throughput_3","latency","reject","mark_tail"]
    ...
    df = df[["ts_record","size","queue_len","prev_queue_len_1","prev_queue_len_2","prev_queue_len_3","prev_latency_1","prev_latency_2","prev_latency_3","prev_throughput_1","prev_throughput_2","prev_throughput_3","latency","reject"]]

    outfile_path = input_path.replace('profile_v1', 'profile_v1_filter')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-output_dir", help="Put the labeled file inside same parent directory, inside 'dataset' folder", default=False, action='store_true')
    ...
    output_dirs = []
    if args.output_dir:
        for dir in trace_profiles:
            path = os.path.join(os.path.dirname(os.path.abspath(dir)), 'dataset')
            output_dirs.append(path)
            create_output_dir(path)
        print("output_dirs:", str(output_dirs))
    else:
        output_dirs = ['None']*len(trace_profiles)
    
    for profile_path, output_dir in zip(trace_profiles, output_dirs):
        print("\nProcessing " + str(profile_path))
        start_processing(profile_path, output_dir)
# How to run:
